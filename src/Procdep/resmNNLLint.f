...
c---- SSend 
     ...
     & fx1z_H(-nf:nf),fx2z_H(-nf:nf),fx1z_L(-nf:nf),fx2z_L(-nf:nf)
      real(dp):: xmsq_noew,msq_noew(-nf:nf,-nf:nf),resmNNLLint_noew
...
! compute weights for scale variation, looping if necessary
      ...
      if (currentPDF == 0) then
        ...
        if (kewcorr /= knone) resmNNLLint_noew=flux*xjac*pswt*xmsq_noew/BrnRat
      endif
