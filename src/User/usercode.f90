...
subroutine userplotter(pjet, wt, wt2, nd)
  ...
  if (jets > 0) then
     ptj = sqrt(pjet(7,1)**2+pjet(7,2)**2)
     etaj = etarap(7,pjet)
  endif
...
function ATLAS_hww2017(ppart) result(res)
  ...
  real(dp) :: ptjveto_track, ptjveto_calo
  real(dp) :: etajveto, etajveto_track, etajveto_calo
...
!     13 TeV cuts ATLAS CONF 016-090
  ...
  if (abs(sqrts - 13000._dp) < 1._dp) then
     ...
     if (VVcut == 3) then
        ...
        if (jets > 0) then
           if     (abs(etaj) > etajveto_calo .or. ptj > ptjveto_calo) then
              passveto = .false.
           elseif (abs(etaj) < etajveto_track .and. ptj > ptjveto_track) then
              passveto = .false.
           endif
        endif
...
function ATLAS_simple(ppart) result(res)
  implicit none
  include 'types.f'
  include 'constants.f'
  include 'npart.f'
